...
<div
  class="blueprint-node builder-node"
  class:selected
  style="border-color: #22c55e; --node-bg: #f0fdf4; --node-dark-bg: #052e16; --node-border: #22c55e;"
  data-testid="node-wf-builder"
>
  ...
  <div class="node-header">
    ...
    <span class="node-type-label">Builder</span>
  </div>

  <div class="node-body">
    <span class="node-name">{data?.label || 'Builder'}</span>
    {#if isLinked}
...
<style>
  .blueprint-node {
    background: var(--node-bg, #f0fdf4);
    border: 2px solid var(--node-border, #22c55e);
    border-radius: 12px;
    padding: 12px;
    min-width: 180px;
    font-size: 13px;
  ...
  :global(.dark) .blueprint-node {
    background: var(--node-dark-bg, #052e16);
  }
  .blueprint-node.selected {
    box-shadow: 0 0 0 2px var(--node-border, #22c55e), 0 4px 12px rgba(0,0,0,0.15);
  }
  .node-header {
    display: flex;
    align-items: center;
