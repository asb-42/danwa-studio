<script>
  /**
   * FactCheckerNode — Fact Checker workflow step.
   *
...
<div
  class="blueprint-node fact-checker-node"
  class:selected
  style="border-color: #10b981; --node-bg: #ecfdf5; --node-dark-bg: #064e3b; --node-border: #10b981;"
  data-testid="node-wf-fact-checker"
>
  ...
  <div class="node-header">
    ...
    <span class="node-type-label">Fact Checker</span>
  </div>

  <div class="node-body">
    <span class="node-name">{data?.label || 'Fact Checker'}</span>
    {#if isLinked}
...
<style>
  ...
  :global(.dark) .blueprint-node {
    background: var(--node-dark-bg, #064e3b);
  }
