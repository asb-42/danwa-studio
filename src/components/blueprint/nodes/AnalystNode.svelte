<script>
  /**
   * AnalystNode — Analyst workflow step.
   *
...
<div
  class="blueprint-node analyst-node"
  class:selected
  style="border-color: #06b6d4; --node-bg: #ecfeff; --node-dark-bg: #0e7490; --node-border: #06b6d4;"
  data-testid="node-wf-analyst"
>
  ...
  <div class="node-header">
    ...
    <span class="node-type-label">Analyst</span>
  </div>

  <div class="node-body">
    <span class="node-name">{data?.label || 'Analyst'}</span>
    {#if isLinked}
...
<style>
  ...
  :global(.dark) .blueprint-node {
    background: var(--node-dark-bg, #0e7490);
  }
