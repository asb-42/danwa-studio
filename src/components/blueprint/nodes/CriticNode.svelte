<script>
  /**
   * CriticNode — Critic workflow step.
   *
...
<div
  class="blueprint-node critic-node"
  class:selected
...
>
  ...
  <div class="node-header">
    ...
    <span class="node-type-label">Critic</span>
  </div>

  <div class="node-body">
    <span class="node-name">{data?.label || 'Critic'}</span>
    {#if isLinked}
...
<style>
  .blueprint-node {
    background: var(--node-bg, #fef2f2);
    border: 2px solid var(--node-border, #ef4444);
    border-radius: 12px;
    padding: 12px;
    min-width: 180px;
    font-size: 13px;
  ...
  :global(.dark) .blueprint-node {
    background: var(--node-dark-bg, #450a0a);
  }
  .blueprint-node.selected {
    box-shadow: 0 0 0 2px var(--node-border, #ef4444), 0 4px 12px rgba(0,0,0,0.15);
  }
  .node-header {
    display: flex;
    align-items: center;
