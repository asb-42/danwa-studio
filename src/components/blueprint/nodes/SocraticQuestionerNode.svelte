...
<div
  class="blueprint-node socratic-questioner-node"
  class:selected
  style="border-color: #8b5cf6; --node-bg: #f5f3ff; --node-dark-bg: #6d28d9; --node-border: #8b5cf6;"
  data-testid="node-wf-socratic-questioner"
>
  ...
  <div class="node-header">
    ...
    <span class="node-type-label">Socratic Questioner</span>
  </div>

  <div class="node-body">
    <span class="node-name">{data?.label || 'Socratic Questioner'}</span>
    {#if isLinked}
...
<style>
  ...
  :global(.dark) .blueprint-node { background: var(--node-dark-bg, #6d28d9); }
  ...
  :global(.config-input-port) {
    ...
    background: white !important; border: 2px solid #f59e0b !important;
    border-radius: 50% !important; top: -5px !important;
    transition: all 0.2s ease;
  }
  :global(.dark) :global(.config-input-port) { background: #6d28d9 !important; }
  :global(.config-input-port):hover { box-shadow: 0 0 0 3px rgba(139, 92, 246, 0.3); }
</style>
